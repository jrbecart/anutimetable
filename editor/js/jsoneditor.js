...
function jsonEditorInit(table_container_id, json_input_container_id, json_output_container_id, json_to_table_btn_id, table_to_json_btn_id, new_line_id, term) {
    ...
    try {
        ...
        // read config File
        //$.get('../data/configTable.json', {}, function (data) {
    ...
    $('#' + table_to_json_btn_id).on('click', function(){
        //send file to save.php
        jsonString = JSON.stringify(makeJson());
        $.ajax({
          url: '../save.php?term=' + term,
...
$(function() {
    ...
    $.contextMenu({
        ...
        items: {
            // functions not needed are commented
...
function makeTable(obj_for_table, counter = 1){
    ...
    if(jQuery.type(obj_for_table) == 'object' || jQuery.type(obj_for_table) == 'array')
